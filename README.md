...
│
└── raw_sensor_inputs.csv   # Iteration log of unfiltered, real distances from anchors
```
...
## 🗃️ Raw Data Logging (Professor / Academic Audit)

For transparent evaluation and academic verification, the `main.py` entrypoint logs all **unmanipulated, raw sensor inputs** at every timestep to `raw_sensor_inputs.csv`.

For each step and node, this CSV tracks the true coordinates against the exact, unmanipulated distance readings `[Dist_A0 ... Dist_A3]` generated by the environment's physical noise models, *before* any algorithmic treatment. This serves as the ground-truth "real input data" fed into the algorithms during simulations.
